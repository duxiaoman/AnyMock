...
# 1. 下载前端代码库（anymock-fe）！！！，并将yml配置文件中的fe.path修改为anymock-fe的dist目录地址,
# 如果前端代码不调整位置,默认是/anymock-fe/dist/ 即可

# 2. 后台运行, 默认使用配置文件anymock-core-config.yml
nohup java -jar anymock-core-runner-1.0-SNAPSHOT.jar >/dev/null 2>/dev/null &

使用自定义配置文件启动, 其中${env}需替换为配置文件环境名
