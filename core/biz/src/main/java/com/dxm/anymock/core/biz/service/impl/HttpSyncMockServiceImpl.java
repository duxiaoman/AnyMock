...
public class HttpSyncMockServiceImpl implements HttpSyncMockService {
    ...
    private InterfaceBranchBO findBranch(String branchName, List<InterfaceBranchBO> branchBOList) {
    ...
    public void mock(HttpMockContext context, HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (configMode == TEXT) {
            ...
            responseBody = groovyService.exec(binding, httpInterfaceBO.getSyncScript()).toString();
        } else if (configMode == GROOVY_TEMPLATE_SWITCH_CASE) {
            Binding binding = buildSyncBinding(request, response);
            String branchName = groovyService.exec(binding, httpInterfaceBO.getBranchJumpScript()).toString();
            logger.info("BranchName = {}", branchName);
            InterfaceBranchBO branchBO = findBranch(branchName, httpInterfaceBO.getBranchScriptList());
            if (branchBO == null) {
                throw new BizException(ResultCode.NOT_FOUND_INTERFACE_BRANCH);
            }
            context.setInterfaceBranchBO(branchBO);

            responseBody = groovyService.exec(binding, branchBO.getSyncScript()).toString();
        } else {
